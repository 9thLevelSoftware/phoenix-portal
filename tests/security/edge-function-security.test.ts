...
describe('Paddle webhook security helpers', () => {
  ...
  it('builds Paddle update bodies for switches, downgrades, and uncancel actions', () => {
    expect(
      buildPaddleSubscriptionPatch('pri_flame_monthly', 'pri_ember_monthly', false),
    ).toEqual({
      action: 'switch',
      body: {
    ...
    expect(
      buildPaddleSubscriptionPatch('pri_flame_monthly', 'pri_flame_annual', true),
    ).toEqual({
      action: 'switch',
      body: {
  ...
  it('maps fully canceled Paddle subscriptions to closed local state', () => {
    const upsert = buildSubscriptionUpsertFromPaddleState({
      ...
      subscription: {
        ...
        scheduled_change: { action: 'cancel', effective_at: '2026-05-17T00:00:00Z' },
      },
