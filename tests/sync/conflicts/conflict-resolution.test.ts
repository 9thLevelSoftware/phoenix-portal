...
describe('Conflict Resolution Integration Tests', () => {
  ...
  describe('Scenario 1: Multi-Device Concurrent Routine Edit', () => {
    ...
    it('should handle concurrent routine edits with LWW', async () => {
      ...
      const deviceAResult = await callPushEndpoint(deviceAPayload, testUser.accessToken);
      ...
      const deviceBResult = await callPushEndpoint(deviceBPayload, testUser.accessToken);
  ...
  describe('Scenario 5: Cross-Entity Integrity', () => {
    ...
    it('should preserve session routine reference when routine is deleted', async () => {
      ...
      const pulledSession = pullBefore.data!.sessions.find(s => s.id === sessionId);
      ...
      const sessionAfter = pullAfter.data!.sessions.find(s => s.id === sessionId);
