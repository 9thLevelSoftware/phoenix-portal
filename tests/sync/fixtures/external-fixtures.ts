...
type ExternalActivityInsert = Database["public"]["Tables"]["external_activities"]["Insert"];
...
export function createStravaActivityFixture(
  ...
  const isCardio = ["Ride", "Run", "Swim", "Walk", "Hike", "VirtualRide", "VirtualRun"].includes(activityType);
...
export function createFitbitActivityFixture(
  ...
  const isCardio = ["Run", "Walk", "Bike", "Swim", "Elliptical", "Treadmill"].includes(activityType);
  ...
  return createExternalActivityFixture({
    ...
    raw_data: {
      ...
      calories: 450,
      caloriesLink: "https://api.fitbit.com/1/user/-/activities/calories/date/2026-04-12.json",
      heartRateZones: [
...
export function createGarminActivityFixture(
  ...
  const isCardio = ["running", "cycling", "swimming", "walking", "hiking", "indoor_cycling", "indoor_running"].includes(activityType);
...
export function createProviderActivitySeries(
  ...
  for (let i = 0; i < count; i++) {
    ...
    switch (provider) {
      case "strava":
        activity = createStravaActivityFixture(activityType as StravaActivityType, {
          user_id: userId,
        ...
        break;
      case "fitbit":
        activity = createFitbitActivityFixture(activityType as FitbitActivityType, {
          user_id: userId,
          started_at: startDate.toISOString(),
          name: `${activityType} Day ${i + 1}`,
        });
        break;
      case "garmin":
        activity = createGarminActivityFixture(activityType as GarminActivityType, {
          user_id: userId,
          started_at: startDate.toISOString(),
...
export function createCorrelatedWeightActivity(
  ...
  externalTime.setMinutes(externalTime.getMinutes() + Math.floor(Math.random() * 30) - 15);
...
export function createHeartRateActivityFixture(
  ...
  return createExternalActivityFixture({
    provider,
    activity_type: provider === "strava" ? "Run" : provider === "fitbit" ? "Run" : "running",
 ...
 */
export function toExternalActivityInsert(row: ExternalActivityRow): ExternalActivityInsert {
