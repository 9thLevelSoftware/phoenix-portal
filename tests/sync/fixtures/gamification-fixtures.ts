...
export const BADGE_DEFINITIONS = {
	...
	ECHO_EXPERT: {
		name: "Echo Expert",
		description: "Complete 50 Echo workouts",
	},
	PUMP_CHAMPION: {
		name: "Pump Champion",
		description: "Complete 50 Pump workouts",
	},

	// Performance badges
	VELOCITY_KING: {
		name: "Velocity King",
		description: "Achieve 1.5 m/s peak velocity",
	},
	POWER_HOUSE: {
		name: "Power House",
		description: "Generate 1000W peak power",
	},
	FORM_PERFECT: {
		name: "Form Perfect",
		description: "Score 95+ form rating 10 times",
	},
	...
	EARLY_BIRD: {
		name: "Early Bird",
		description: "Complete 20 workouts before 7am",
	},
	NIGHT_OWL: {
		name: "Night Owl",
		description: "Complete 20 workouts after 10pm",
	},
} as const;
 ...
 */
export function createAggregateGamificationFixture(
	options: { userId?: string; badgeCount?: number; prCount?: number } = {},
): AggregateGamificationFixture {
	const { userId = DEFAULT_USER_ID, badgeCount = 5, prCount = 10 } = options;
...
export function toRpgAttributesInsert(
	row: RpgAttributesRow,
): RpgAttributesInsert {
	const { id, updated_at, ...rest } = row;
	return rest;
}
...
export function toGamificationStatsInsert(
	row: GamificationStatsRow,
): GamificationStatsInsert {
	const { id, updated_at, ...rest } = row;
	return rest;
}
