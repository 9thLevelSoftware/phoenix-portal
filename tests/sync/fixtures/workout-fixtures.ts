...
export function createRepTelemetrySeriesFixture(
	...
	for (let i = 0; i < sampleCount; i++) {
		...
		const position =
			phase === "concentric" ? phaseProgress * 800 : (1 - phaseProgress) * 800;
 ...
 */
export function createNestedSessionFixture(
	options: {
 ...
 */
export function toSessionInsert(row: WorkoutSessionRow): WorkoutSessionInsert {
	const { id, ...rest } = row;
	return rest;
}
