...
type TrainingCycleInsert =
	Database["public"]["Tables"]["training_cycles"]["Insert"];
 ...
 */
export function createNestedCycleFixture(
	options: {
	...
	const _totalDays = durationWeeks * 7;
		...
		for (let dayOfWeek = 0; dayOfWeek < 7; dayOfWeek++) {
			...
			if (isWorkoutDay && workoutsThisWeek < workoutDaysPerWeek) {
				...
				if (isDeloadWeek) {
					...
					days.push(
						createWorkoutDayFixture(cycleId, dayNumber, routineId, {
...
export function createPPLCycleFixture(
	...
	for (let week = 1; week <= 4; week++) {
		...
		// Day 1: Push
		days.push(
			createWorkoutDayFixture(cycleId, weekOffset + 1, routineIds.push, {
		...
		// Day 2: Pull
		days.push(
			createWorkoutDayFixture(cycleId, weekOffset + 2, routineIds.pull, {
				weight_adjustment: weightAdj,
				rep_modifier: repMod,
			}),
		);
		// Day 3: Legs
		days.push(
			createWorkoutDayFixture(cycleId, weekOffset + 3, routineIds.legs, {
				weight_adjustment: weightAdj,
				rep_modifier: repMod,
			}),
		);
		// Day 4: Push
		days.push(
			createWorkoutDayFixture(cycleId, weekOffset + 4, routineIds.push, {
				weight_adjustment: weightAdj,
				rep_modifier: repMod,
			}),
		);
		// Day 5: Pull
		days.push(
			createWorkoutDayFixture(cycleId, weekOffset + 5, routineIds.pull, {
				weight_adjustment: weightAdj,
				rep_modifier: repMod,
			}),
		);
		// Day 6: Legs
		days.push(
			createWorkoutDayFixture(cycleId, weekOffset + 6, routineIds.legs, {
				weight_adjustment: weightAdj,
				rep_modifier: repMod,
			}),
		);
