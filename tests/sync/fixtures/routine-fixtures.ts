 ...
 */
export function createNestedRoutineFixture(options: {
  ...
  if (includeSuperset && exerciseCount >= 2) {
    const supersetExercises = createSupersetExercises(
      routineId,
      {
        ...
        muscleGroups: [exerciseTemplates[0].muscleGroup, exerciseTemplates[1].muscleGroup],
      },
      orderIndex
    );
  ...
  for (let i = 0; i < remainingCount; i++) {
    const template = exerciseTemplates[(startIndex + i) % exerciseTemplates.length];
