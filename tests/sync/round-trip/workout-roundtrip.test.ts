...
describe("Workout Round-Trip Tests", () => {
	const _testUserId = `test-user-${Date.now()}`;
	...
	describe("Nested Session Hierarchy", () => {
		it("should preserve nested exercise hierarchy through round-trip", async () => {
			...
			const pulledExercise2 = pulledSession.exercises.find(
				(e) => e.id === exercise2Id,
			);
		...
		it("should handle complete 4-level nested hierarchy", async () => {
			...
			for (let e = 0; e < 2; e++) {
				...
				exercises.push(
					createTestExercise(sessionId, e, {
						id: exerciseId,
			...
			// Verify total counts
			const totalSets = pulledSession.exercises.reduce(
			...
			const totalReps = pulledSession.exercises.reduce(
				(sum, ex) =>
					sum +
					ex.sets.reduce((setSum, set) => setSum + set.repSummaries.length, 0),
				0,
			);
	...
	describe("Delta Sync via updated_at", () => {
		it("should return sessions updated after lastSync timestamp", async () => {
			...
			const pushResult1 = await callPushEndpoint(
				payload1,
			...
			expect(sync1Time).toEqual(expect.any(Number));
			if (typeof sync1Time !== "number") {
				throw new Error("Expected first push to return syncTime");
			}
		...
		it("should return empty when no updates after lastSync", async () => {
			...
			const syncTime = pushResult.data?.syncTime;
			expect(syncTime).toEqual(expect.any(Number));
			if (typeof syncTime !== "number") {
				throw new Error("Expected push to return syncTime");
			}
			const futureTime = syncTime + 10000;
