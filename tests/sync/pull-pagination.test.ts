...
function seedSessions(userId: string, count: number): SessionDto[] {
	return Array.from({ length: count }, (_, i) =>
		createTestSession(userId, {
			name: `Session ${i}`,
			startedAt: new Date(
				Date.parse("2026-04-01T00:00:00Z") + i * 1000,
			).toISOString(),
			exercises: [],
		}),
	);
...
describe("mobile-sync-pull pagination", () => {
	...
	describe("Default page size", () => {
		it.skip("returns hasMore=true with nextCursor when >DEFAULT_PAGE_SIZE sessions match — requires live Edge Function", async () => {
	...
	describe("Multi-page traversal (>500 entities)", () => {
		it.skip("union of all pages equals the full set with no duplicates — requires live Edge Function", async () => {
	...
	describe("Empty delta (lastSync at or after latest push)", () => {
		...
		it.skip("asserts hasMore=false and nextCursor is absent/undefined in empty-delta pull — requires live Edge Function", async () => {
	...
	describe("Large knownEntityIds list (>MAX_PARITY_IDS = 500)", () => {
		it.skip("skips parity filter for entity types exceeding MAX_PARITY_IDS — requires live Edge Function", async () => {
			...
			const knownIds = Array.from({ length: 600 }, () => crypto.randomUUID());
			const result = await callPullEndpoint(0, testUser.accessToken, {
