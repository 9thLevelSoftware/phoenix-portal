...
export function mockPushEndpoint(
	...
	if (batchError) {
		return batchError as EdgeFunctionResult<{
			success: boolean;
			syncTime?: number;
		}>;
	}
	...
	if (incompleteRoutineIds.length > 0) {
		const formatted =
			formatPushPayloadIncompleteRoutinesError(incompleteRoutineIds);
		return {
			success: false,
			status: 400,
			error: {
 ...
 */
export function getMockRoutine(
	routineId: string,
): RoutineResponseDto | undefined {
	...
	if (
		...
		return {
			...
			error: {
				...
				code:
					batchFailureConfig.errorCode === 429
						? "RATE_LIMITED"
						: "SERVER_ERROR",
			},
		};
