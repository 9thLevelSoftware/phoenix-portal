...
describe("Server-Side Validation Invariants", () => {
	...
	describe("Payload size (>10 MB)", () => {
		it.skip("rejects payloads > 10 MB with 413 — requires live Edge Function", async () => {
	...
	describe("Array size caps", () => {
		it.skip("rejects sessions.length > 10_000 with 400 — requires live Edge Function", async () => {
		...
		it.skip("rejects telemetry.length > 50_000 with 400 — requires live Edge Function", async () => {
		...
		it.skip("rejects routines.length > 10_000 with 400 — requires live Edge Function", async () => {
		...
		it.skip("rejects cycles.length > 10_000 with 400 — requires live Edge Function", async () => {
			// Enforced in mobile-sync-push/index.ts against MAX_ENTITIES_PER_TYPE.
	...
	describe("Rate limit (10 req/min per user on push, 11th → 429)", () => {
		it.skip("returns 429 with Retry-After header on 11th push inside 60s — requires live rate limiter", async () => {
	...
	describe("Subscription tier gating (EMBER or higher on push)", () => {
		it.skip("rejects FREE tier user with 402/403 on push — requires live subscription lookup", async () => {
		...
		it.skip("rejects FREE tier user with 402/403 on pull — requires live subscription lookup", async () => {
	...
	describe("Authentication", () => {
		...
		it.skip("rejects expired JWT with 401 and AUTH error signal — requires live Supabase auth", async () => {
