...
async function computeGlobalRankings(
	...
	const { data: prRankings, error: prError } = await supabase.rpc(
	...
	const { data: masteryRankings, error: masteryError } = await supabase.rpc(
...
async function computeWeeklyRankings(
	...
	if (
		metricConfig.metric === "total_volume_kg" ||
		...
		for (const session of sessions ?? []) {
			const existing = weeklyStats.get(session.user_id) ?? {
				volume: 0,
				count: 0,
			};
...
async function computeUserRankings(
	...
	const volumeValues = eligibleUserIds.map(
		(id) => statsMap.get(id)?.total_volume_kg ?? 0,
	);
	const volumeRank = calculateRank(
		userStats?.total_volume_kg ?? 0,
		volumeValues,
	);
	rankings.push({
		metric: "totalVolume",
	...
	const workoutValues = eligibleUserIds.map(
		(id) => statsMap.get(id)?.total_workouts ?? 0,
	);
	const workoutRank = calculateRank(
	...
	const longestStreakValues = eligibleUserIds.map(
		(id) => statsMap.get(id)?.longest_streak ?? 0,
	);
	const longestStreakRank = calculateRank(
	...
	const currentStreakValues = eligibleUserIds.map(
		(id) => statsMap.get(id)?.current_streak ?? 0,
	);
	const currentStreakRank = calculateRank(
	...
	const masteredCount = [...exerciseSessionMap.values()].filter(
		(sessions) => sessions.size >= 10,
	).length;
