...
Deno.serve(async (req) => {
	...
	try {
		...
		if (stateError || !stateRow) {
			return Response.redirect(
				`${APP_URL()}/integrations?error=invalid_state`,
				302,
			);
		}
		...
		if (new Date(stateRow.expires_at) < new Date()) {
			...
			return Response.redirect(
				`${APP_URL()}/integrations?error=state_expired`,
				302,
			);
		}
		...
		if (!tokenResponse.ok) {
			console.error(
				"Strava token exchange failed: status",
		...
		// ----------------------------------------------------------------
		return Response.redirect(`${APP_URL()}/integrations?connected=strava`, 302);
	} catch (err) {
