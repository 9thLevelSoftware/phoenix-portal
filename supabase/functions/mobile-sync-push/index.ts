...
async function assertChildRowsOwnedViaParent(
	...
	for (let i = 0; i < unique.length; i += chunkSize) {
		...
		if (childErr) {
			throw new Error(
				`Ownership check on ${childTable} failed: ${childErr.message}`,
			);
		}
		...
		if (parentErr) {
			throw new Error(
				`Ownership check on ${parentTable} failed: ${parentErr.message}`,
			);
		}
...
Deno.serve(async (req) => {
	...
	try {
		...
		if (payload.sessions && payload.sessions.length > MAX_ENTITIES_PER_TYPE) {
			return new Response(
				JSON.stringify({
					error: `Too many sessions. Maximum is ${MAX_ENTITIES_PER_TYPE}.`,
				}),
				{
					status: 400,
					headers: { ...cors, "Content-Type": "application/json" },
		...
		if (payload.telemetry && payload.telemetry.length > MAX_TELEMETRY_POINTS) {
			return new Response(
				JSON.stringify({
					error: `Too many telemetry items. Maximum is ${MAX_TELEMETRY_POINTS}.`,
				}),
				{
					status: 400,
					headers: { ...cors, "Content-Type": "application/json" },
		...
		if (payload.routines && payload.routines.length > MAX_ENTITIES_PER_TYPE) {
			return new Response(
				JSON.stringify({
					error: `Too many routines. Maximum is ${MAX_ENTITIES_PER_TYPE}.`,
				}),
				{
					status: 400,
					headers: { ...cors, "Content-Type": "application/json" },
		...
		if (payload.cycles && payload.cycles.length > MAX_ENTITIES_PER_TYPE) {
			return new Response(
				JSON.stringify({
					error: `Too many cycles. Maximum is ${MAX_ENTITIES_PER_TYPE}.`,
				}),
				{
					status: 400,
					headers: { ...cors, "Content-Type": "application/json" },
		...
		if (allProfiles && allProfiles.length > 0) {
			...
			if (upsertError) {
				...
				if (localProfileId) {
					console.warn(
						"Clearing localProfileId to avoid FK violation on session insert",
					);
					localProfileId = null;
				}
			} else {
			...
			const profileName =
				payload.profileName ??
				(localProfileId === "default" ? "Default" : "Profile");
		...
		const allRepSummaryIds = (payload.sessions ?? []).flatMap((s) =>
			s.exercises.flatMap((e) =>
				e.sets.flatMap((st) => st.repSummaries.map((r) => r.id)),
			),
		);
		...
		const allCycleDayIds = (payload.cycles ?? []).flatMap((c) =>
			c.days.map((d) => d.id),
		);
		...
		const _cycleIdSet = new Set(allCycleIds);
			...
			for (const e of s.exercises) {
				if (e.sessionId !== s.id) {
					return fkMismatchResponse(
						`exercise ${e.id} sessionId must equal parent session ${s.id}`,
					);
				}
				for (const st of e.sets) {
					if (st.exerciseId !== e.id) {
						return fkMismatchResponse(
							`set ${st.id} exerciseId must equal parent exercise ${e.id}`,
						);
					}
					for (const r of st.repSummaries) {
						if (r.setId !== st.id) {
							return fkMismatchResponse(
								`rep_summary ${r.id} setId must equal parent set ${st.id}`,
							);
						}
					}
				}
		...
		for (const [table, ids] of directOwnerChecks) {
			const blocked = await assertRowsOwnedByUser(
				supabase,
				table,
				ids,
				userId,
		...
		const dayRoutineIdsToVerify = (payload.cycles ?? [])
			...
			.filter(
				(rid): rid is string =>
					typeof rid === "string" && rid.length > 0 && !routineIdSet.has(rid),
			);
		...
		const childAllowed = <_T>(
			parentSet: Set<string> | null,
			parentId: string,
		): boolean => parentSet === null || parentSet.has(parentId);
		...
		if (payload.sessions && payload.sessions.length > 0) {
			...
			if (affectedSessionIds.length > 0) {
				...
				if (cleanupErr)
					throw new Error(
						`exercises pre-cleanup failed: ${cleanupErr.message}`,
					);
			}
			...
			if (repRows.length > 0) {
				...
				if (repErr)
					throw new Error(`rep_summaries upsert failed: ${repErr.message}`);
			...
			if (payload.telemetry && payload.telemetry.length > 0) {
				...
				for (let i = 0; i < payload.telemetry.length; i += TELEMETRY_BATCH) {
					const batch = payload.telemetry
						.slice(i, i + TELEMETRY_BATCH)
						.map((t) => ({
							id: t.id,
							set_id: t.setId,
							user_id: userId,
					...
					if (telErr)
						throw new Error(`rep_telemetry upsert failed: ${telErr.message}`);
			...
			if (progressRows.length > 0) {
				...
				const { data: existingProgress, error: existingProgressErr } =
					await supabase
				...
				if (existingProgressErr) {
					throw new Error(
						`exercise_progress lookup failed: ${existingProgressErr.message}`,
					);
				}

				const existingProgressKeys = new Set(
					(existingProgress ?? []).map(
						(row) => `${row.session_id}:${row.exercise_name}`,
					),
				);
				...
				if (dedupedProgressRows.length > 0) {
					...
					if (progErr)
						throw new Error(
							`exercise_progress insert failed: ${progErr.message}`,
						);
			...
			if (prRows.length > 0) {
				const achievedAtValues = [
					...new Set(prRows.map((row) => row.achieved_at as string)),
				];
				const { data: existingPrs, error: existingPrErr } = await supabase
					.from("personal_records")
					.select(
						"exercise_name, achieved_at, value, record_type, workout_phase",
					)
					.eq("user_id", userId)
					.in("achieved_at", achievedAtValues);
				if (existingPrErr) {
					throw new Error(
						`personal_records lookup failed: ${existingPrErr.message}`,
					);
				}
				...
				const existingPrKeys = new Set(
					(existingPrs ?? []).map(
						(row) =>
							`${profileTag}:${row.exercise_name}:${row.achieved_at}:${row.value}:${row.record_type}:${row.workout_phase ?? "COMBINED"}`,
					),
				);
				...
				if (dedupedPrRows.length > 0) {
					...
					if (prErr)
						throw new Error(`personal_records insert failed: ${prErr.message}`);
		...
		if (payload.routines && payload.routines.length > 0) {
			...
			if (SYNC_LWW_ENABLED) {
				...
				for (const rr of (lwwData ?? []) as LwwUpsertRow[]) {
					if (rr.accepted) acceptedRoutineIds.add(rr.id);
						...
						rejections.routines.push({
							id: rr.id,
							serverUpdatedAt: rr.server_updated_at,
						});
				}
			...
			if (reRows.length > 0) {
				...
				if (reErr)
					throw new Error(`routine_exercises upsert failed: ${reErr.message}`);
			}
			...
			for (const routineId of routineIds) {
				...
				if (idsForRoutine.length > 0) {
					...
					if (orphanErr)
						console.warn(
							`routine_exercises orphan cleanup warning for ${routineId}:`,
							orphanErr.message,
						);
				} else {
		...
		if (payload.cycles && payload.cycles.length > 0) {
			...
			if (SYNC_LWW_ENABLED) {
				...
				if (lwwErr)
					throw new Error(`training_cycles LWW RPC failed: ${lwwErr.message}`);
				...
				if (cycErr)
					throw new Error(`training_cycles upsert failed: ${cycErr.message}`);
			...
			if (dayRows.length > 0) {
				...
				if (dayErr)
					throw new Error(`cycle_days upsert failed: ${dayErr.message}`);
			}
			...
			for (const cycle of payload.cycles) {
				...
				if (orphanErr)
					console.warn(
						`cycle_days orphan cleanup warning for ${cycle.id}:`,
						orphanErr.message,
					);
			}
		}
		...
		if (payload.rpgAttributes) {
			...
			if (SYNC_LWW_ENABLED) {
				...
				if (lwwErr)
					throw new Error(`rpg_attributes LWW RPC failed: ${lwwErr.message}`);
				for (const rr of (lwwData ?? []) as LwwUpsertRow[]) {
					if (!rr.accepted)
						rejections.rpgAttributes.push({
							id: rr.id,
							serverUpdatedAt: rr.server_updated_at,
						});
				}
				...
				if (rpgErr)
					throw new Error(`rpg_attributes upsert failed: ${rpgErr.message}`);
			}
		}
		...
		if (payload.badges && payload.badges.length > 0) {
			...
			if (badgeErr)
				throw new Error(`earned_badges upsert failed: ${badgeErr.message}`);
		...
		if (payload.gamificationStats) {
			...
			if (SYNC_LWW_ENABLED) {
				...
				if (lwwErr)
					throw new Error(
						`gamification_stats LWW RPC failed: ${lwwErr.message}`,
					);
				for (const rr of (lwwData ?? []) as LwwUpsertRow[]) {
					if (!rr.accepted)
						rejections.gamificationStats.push({
							id: rr.id,
							serverUpdatedAt: rr.server_updated_at,
						});
				}
				...
				if (gsErr)
					throw new Error(`gamification_stats upsert failed: ${gsErr.message}`);
			}
		}
		...
		if (payload.assessments && payload.assessments.length > 0) {
			...
			const existingKeys = new Set(
				(existingAssess ?? []).map(
					(r: Record<string, unknown>) => `${r.exercise_id}:${r.created_at}`,
				),
			);
		...
		if (payload.externalActivities && payload.externalActivities.length > 0) {
			...
			if (SYNC_LWW_ENABLED) {
				...
				if (lwwErr) {
					...
					const byIdx = new Map<
						string,
						{ externalId: string; provider: string }
					>();
				...
				if (extErr) {
					...
					externalActivityKeys = (extData ?? []).map(
		...
		const status =
			message.includes("upsert failed") || message.includes("insert failed")
				? 400
				: 500;
