  ...
  try {
    ...
    if (liveCycles.length > 0) {
      ...
      for (const row of (Array.isArray(mergeData) ? mergeData : []) as CycleMergeRow[]) {
        if (row.accepted) {
          acceptedIds.add(normalizeUuid(row.id));
          ...
          if (row.structure_applied && row.server_updated_at) {
            cycleVersions[normalizeUuid(row.id)] = row.server_updated_at;
          }
        } else {
      ...
      if (racedCycleIds.length > 0) {
        ...
        for (const racedId of racedCycleIds) delete cycleVersions[normalizeUuid(racedId)];
      }
    }
    ...
    if (externalActivities.length > 0) {
      ...
      if (SYNC_LWW_ENABLED) {
        ...
        if (lwwErr) {
          ...
          // PostgreSQL back to the mobile row case-insensitively.
