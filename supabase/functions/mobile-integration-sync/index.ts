		...
		if (action === "connect") {
			if (!apiKey || typeof apiKey !== "string" || apiKey.trim().length === 0) {
				return new Response(
					JSON.stringify({
						status: "error",
						error: "API key is required for connect action",
					}),
					{
						status: 400,
						headers: { ...cors, "Content-Type": "application/json" },
...
async function persistActivities(
	...
	for (const activity of activities) {
		const { error } = await supabase.from("external_activities").upsert(
			{
				user_id: userId,
				...
				duration_seconds:
					activity.durationSeconds > 0 ? activity.durationSeconds : null,
		...
		if (error) {
			console.warn(
				`Failed to persist activity ${activity.externalId}:`,
				error.message,
			);
		}
	}
