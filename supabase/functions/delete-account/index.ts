	...
	try {
		...
		try {
			...
			if (avatarFiles && avatarFiles.length > 0) {
				...
				console.log(
					`Removed ${filePaths.length} avatar file(s) for user ${userId}`,
				);
			}
		} catch (storageErr) {
			// Log but continue — avatar cleanup is not critical
			console.error(
				"Storage cleanup error (continuing with deletion):",
				storageErr,
			);
		}
		...
		if (shouldCancelPaddle && !paddleApiKey) {
			...
			return new Response(
				JSON.stringify({
					error: "Billing service not configured. Account deletion aborted.",
				}),
				{
					status: 502,
					headers: { ...cors, "Content-Type": "application/json" },
		...
		if (updateError) {
			console.error(
				"[DELETE_ACCOUNT] Failed to mark deletion request as executed:",
				updateError,
			);
			return new Response(
				JSON.stringify({
					error: "Failed to process deletion. Please try again.",
				}),
				{
					status: 500,
					headers: { ...cors, "Content-Type": "application/json" },
		...
		if (deleteError) {
			...
			console.error(
				"[DELETE_ACCOUNT] Failed to delete auth user, rolled back request status:",
				deleteError,
			);
			return new Response(
				JSON.stringify({
