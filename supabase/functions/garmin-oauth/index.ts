	...
	try {
		...
		if (!oauthToken && !oauthVerifier) {
			...
			if (!requestTokenResponse.ok) {
				console.error(
					"Garmin request token failed: status",
					requestTokenResponse.status,
				);
				return Response.redirect(
					`${APP_URL}/integrations?error=garmin_request_token_failed`,
				);
			}
		...
		if (oauthToken && oauthVerifier) {
			...
			if (!accessTokenResponse.ok) {
				console.error(
					"Garmin access token exchange failed: status",
					accessTokenResponse.status,
				);
				return Response.redirect(
					`${APP_URL}/integrations?error=garmin_token_exchange_failed`,
				);
			}
