 ...
 * `oauth_tokens`. Charging those to the pull budget means three mistyped keys
 * lock the user out of saving a corrected one for 15 minutes with no way back.
 * They also carry no provider-quota argument: an API key is per user, unlike
 * Strava's application-wide read quota. A separate, roomier bucket keeps the
 * abuse ceiling while leaving a typo-retry loop usable.
 */
