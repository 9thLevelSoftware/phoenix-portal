...
// These are generated by the client via generateUUID() and are always UUIDs.
...
function validateKnownEntityIds(
	...
	for (const id of k.sessionIds ?? []) {
		if (typeof id !== "string" || !UUID_REGEX.test(id)) {
			return new Response(
				JSON.stringify({ error: "Invalid knownEntityIds.sessionIds entry" }),
				{
					status: 400,
	...
	for (const id of k.routineIds ?? []) {
		if (typeof id !== "string" || !UUID_REGEX.test(id)) {
			return new Response(
				JSON.stringify({ error: "Invalid knownEntityIds.routineIds entry" }),
				{
					status: 400,
					headers: { ...cors, "Content-Type": "application/json" },
				},
	...
	for (const id of k.cycleIds ?? []) {
		if (typeof id !== "string" || !UUID_REGEX.test(id)) {
			return new Response(
				JSON.stringify({ error: "Invalid knownEntityIds.cycleIds entry" }),
				{
					status: 400,
					headers: { ...cors, "Content-Type": "application/json" },
				},
	...
	for (const id of k.badgeIds ?? []) {
		...
		if (!Number.isInteger(n) || n <= 0) {
			return new Response(
				JSON.stringify({ error: "Invalid knownEntityIds.badgeIds entry" }),
				{
					status: 400,
					headers: { ...cors, "Content-Type": "application/json" },
				},
	...
	for (const id of k.personalRecordIds ?? []) {
		...
		if (!Number.isInteger(n) || n <= 0) {
			return new Response(
				JSON.stringify({
					error: "Invalid knownEntityIds.personalRecordIds entry",
				}),
				{
					status: 400,
					headers: { ...cors, "Content-Type": "application/json" },
