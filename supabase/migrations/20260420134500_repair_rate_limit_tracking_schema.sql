-- Repair schema drift on rate_limit_tracking for the linked production project.
--
-- Production logs show `check_rate_limit` failing with:
--   column "key" does not exist
-- even though the earlier migration history claims the per-user rate-limit
-- columns were already shipped. Treat this as live schema drift and repair it
-- idempotently here so prod no longer depends on the historical migration
-- having applied correctly.
...
ALTER TABLE public.rate_limit_tracking
  DROP CONSTRAINT IF EXISTS rate_limit_tracking_provider_key;
...
CREATE INDEX IF NOT EXISTS idx_rate_limit_key_user_window
  ON public.rate_limit_tracking (key, user_id, window_started_at);
...
BEGIN
  ...
  LOOP
    SELECT rlt.*
      INTO v_row
      FROM public.rate_limit_tracking rlt
      WHERE rlt.key = p_key
        AND rlt.user_id = p_user_id
      FOR UPDATE;
    ...
    ELSE
      IF v_row.window_started_at < (v_now - v_window_interval) THEN
        UPDATE public.rate_limit_tracking rlt
        SET
          requests_this_window = 1,
          window_started_at = v_now,
          last_request_at = v_now,
          last_reset_at = v_now,
          provider = COALESCE(rlt.provider, p_key)
        WHERE rlt.id = v_row.id;
