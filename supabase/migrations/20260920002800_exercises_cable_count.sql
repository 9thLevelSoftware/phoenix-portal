-- FP-4 / F-025 / A-017: carry the per-exercise cable count through push,
-- storage and pull, so the portal can show per-cable load (as the phone does)
-- next to total load = per-cable x the actual cable count, without doubling
-- single-cable sessions (display is PR 30; mobile sends the value in PR 29).
--
...
--    signature is unchanged (7 args, p_progress JSONB DEFAULT NULL), so this
--    is a signature-preserving CREATE OR REPLACE and exactly one overload
--    remains (KD-3 rule 3 does not require a drop).
--
-- KD-3 rule 3a: the body is PR 24's (20260920002400_progress_refresh_and_
-- indexes.sql) verbatim - including PR 20's FINAL RE-LINK KEY telemetry
-- preservation (steps 0 and 6) and PR 24's exercise_progress refresh (step 7)
-- - except step 2, which gains the cable_count column in its INSERT column
-- list, SELECT list, record definition and ON CONFLICT update.
-- Chain: PR 20 -> PR 24 -> PR 28.
-- KD-3 rule 3b: REVOKE ALL FROM PUBLIC/anon/authenticated, GRANT to
-- service_role only.

ALTER TABLE public.exercises
  ADD COLUMN IF NOT EXISTS cable_count SMALLINT
  CONSTRAINT exercises_cable_count_check CHECK (cable_count IN (1, 2));

COMMENT ON COLUMN public.exercises.cable_count IS
  'Cables used for this exercise (1 or 2), as reported by mobile. NULL = unknown (older builds); never treat NULL as 2.';
