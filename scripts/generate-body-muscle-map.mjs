...
const sourcePath = path.join(root, "supabase", "seed-data", "exercise_dump.json");
		...
		return {
			...
			sourceMuscles: [
				...new Set([
					...
					...(exercise.muscleGroups ?? []).map((value) => String(value).trim().toUpperCase()),
				].filter(Boolean)),
			],
			bodyMuscles: buildBodyMuscles(exercise),
		};
	})
