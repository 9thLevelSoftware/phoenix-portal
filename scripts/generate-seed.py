#!/usr/bin/env python3
"""
Generate comprehensive seed SQL for Phoenix Portal from a Phoenix app backup export.

Reads: C:/Users/dasbl/Downloads/Phoenix App Backup (2).json
...
EXPORT_PATH = "C:/Users/dasbl/Downloads/Phoenix App Backup (2).json"
...
L("    RAISE NOTICE 'No user found in auth.users; skipping seed data load.';")
L("    RETURN;")
L("  END IF;")
L("")
...
for i, routine in enumerate(share_candidates[:2]):
    ...
    tags = ["phoenix", name.lower()]
    tags_sql = "ARRAY[" + ",".join(f"'{t}'" for t in tags) + "]::text[]"

    desc = f"My {name.lower()} routine for the Phoenix fitness machine. Tested and refined over weeks of training."
    L(
...
L(
    f"  VALUES (sc1, uid, tc1, '{cycle_name} Program', 'A full-body weekly rotation designed for the Phoenix fitness machine. Hits every muscle group with strategic rest days.', 4, ARRAY['full body', 'weekly', 'phoenix']::text[], 'Intermediate', {random.randint(5, 20)}, {random.randint(2, 10)}, {round(random.uniform(10, 30), 2)}, '{sql_ts(NOW - timedelta(days=7))}');"
)
L("")
