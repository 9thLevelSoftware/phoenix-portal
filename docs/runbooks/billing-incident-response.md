...
### Find annual subscriptions affected by a bad recurring discount

Use this when a Paddle discount was intended to mean "two months free" but was
configured as two recurring billing periods. For annual prices, the second
billing period is another year, so changing only `next_billed_at` still previews
`$0.00` while the discount remains attached.

1. Disable new checkout redemptions for the discount in Paddle, or use the script
   with `--disable-checkout` during the apply step below.
2. Identify affected annual subscriptions from webhook/order logs or Paddle:
   - `discount.id` equals the bad discount ID.
   - `items[].price.id` is one of the annual price IDs.
   - status is not `past_due`.
3. Email affected users before moving a yearly renewal forward to the intended
   two-month beta end date.

Create a newline-separated subscription list such as `affected-subscriptions.txt`,
then preview the exact Paddle update:

```bash
PADDLE_API_KEY=... npm run billing:paddle:correct-discount -- \
  --environment production \
  --discount-id "$BAD_DISCOUNT_ID" \
  --annual-price-id "$PADDLE_EMBER_ANNUAL_PRICE_ID" \
  --annual-price-id "$PADDLE_FLAME_ANNUAL_PRICE_ID" \
  --annual-price-id "$PADDLE_INFERNO_ANNUAL_PRICE_ID" \
  --subscriptions-file affected-subscriptions.txt \
  --next-billed-at 2026-08-09T00:00:00Z \
  --disable-checkout
```

If the previews are correct, apply the change:
...
The script previews `PATCH /subscriptions/{subscription_id}/preview` before any
subscription update. The applied patch is intentionally limited to:
...
Do not run this against subscriptions that no longer have the bad discount or
that are on monthly prices. The script skips those cases by default.
