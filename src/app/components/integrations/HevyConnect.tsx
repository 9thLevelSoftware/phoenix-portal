...
import { Dumbbell, Upload, Key, CheckCircle, AlertCircle, FileText } from 'lucide-react';
 ...
 */
export function HevyConnect({ userId, isConnected, onDisconnect }: HevyConnectProps) {
  // API key state
  ...
  // CSV import state
  const [parsedActivities, setParsedActivities] = useState<NormalizedActivity[] | null>(null);
    ...
    reader.onload = (e) => {
      try {
        ...
        if (activities.length === 0) {
          toast.error('No workouts found in CSV. Verify this is a Hevy export file.');
        ...
        toast.error('Failed to parse CSV file. Make sure it is a valid Hevy export.');
        setParsedActivities(null);
      }
    };
    reader.onerror = () => {
  ...
  const handleImport = useCallback(async () => {
    ...
    try {
      ...
      toast.error(err instanceof Error ? err.message : 'Failed to import workouts');
    } finally {
        ...
        <Tabs defaultValue="csv">
          ...
          <TabsContent value="csv" className="space-y-4 mt-4">
            <p className="text-sm text-muted-foreground">
              Export your workouts from Hevy (Settings &rarr; Export Data) and upload the CSV file.
            </p>
            ...
            {parsedActivities && parsedActivities.length > 0 && (
              <div className="rounded-lg border border-border/50 bg-card/50 p-4 space-y-3">
                <div className="flex items-center gap-2 text-sm font-medium">
                  ...
                  Import Preview
                  {csvFileName && (
                    <span className="text-muted-foreground font-normal">({csvFileName})</span>
                  )}
                </div>
                <div className="grid grid-cols-3 gap-3 text-sm">
                  ...
                  <div>
                    ...
                    <p className="font-semibold text-xs">{getDateRange(parsedActivities)}</p>
                  </div>
                  <div>
                    <p className="text-muted-foreground text-xs">Total Duration</p>
                    <p className="font-semibold">{getTotalDuration(parsedActivities)}</p>
                  </div>
                </div>
                <div className="flex gap-2">
                  ...
                  >
                    {isImporting ? 'Importing...' : `Import ${parsedActivities.length} Workouts`}
                  </Button>
                  <Button
