...
import { calculateAsymmetry, ASYMMETRY_THRESHOLD } from '@/lib/biomechanics';
...
function getAsymmetry(rep: RepSummary): number {
  if (rep.asymmetry_pct != null && rep.asymmetry_pct !== 0) return rep.asymmetry_pct;
    ...
    <>
      <svg width={width} height={height}>
        <Group left={margin.left} top={margin.top}>
          ...
          <AxisBottom
            top={innerHeight}
            scale={xScale}
            tickValues={[-20, -10, 0, 10, 20].filter((v) => Math.abs(v) <= maxAbs)}
      ...
      {tooltipOpen && tooltipData && (
        <TooltipWithBounds left={tooltipLeft} top={tooltipTop} style={{
          ...
          <div style={{ fontWeight: 600, marginBottom: 4 }}>Rep {tooltipData.repNumber}</div>
          ...
          <div style={{ color: tooltipData.isBalanced ? COLOR_BALANCED : COLOR_IMBALANCED }}>
  ...
  return (
    <div className="flex flex-col items-center gap-4 py-6">
      ...
      <div className="w-full max-w-xs">
        <div className="mb-1 flex justify-between text-xs" style={{ color: COLOR_TEXT }}>
  ...
  return (
    <div style={{ position: 'relative', height }}>
      <ParentSize>
        {({ width }) =>
          width > 0 ? (
            <PerRepChart repSummaries={repSummaries} width={width} height={height} />
          ) : null
        }
      </ParentSize>
    </div>
