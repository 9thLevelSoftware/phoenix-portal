...
const TIERS: TierConfig[] = [
  ...
  },
  {
    ...
    buttonClass: 'bg-gradient-to-r from-[#FF6B35] to-[#DC2626] hover:from-[#FF6B35]/90 hover:to-[#DC2626]/90 text-white border-0',
    popular: true,
  },
  {
    name: 'Elite',
    ...
    buttonClass: 'bg-gradient-to-r from-[#F59E0B] to-[#B45309] hover:from-[#F59E0B]/90 hover:to-[#B45309]/90 text-black border-0',
  },
];
    ...
    <div className="min-h-screen p-4 md:p-8">
      <div className="max-w-5xl mx-auto">
        ...
        <div className="flex items-center justify-center gap-3 mb-10">
          <span className={`text-sm font-medium ${!isAnnual ? 'text-white' : 'text-[#9CA3AF]'}`}>
            Monthly
          </span>
          <Switch
            checked={isAnnual}
              ...
              >
                ...
                {isCurrent && (
                  <div className="absolute -top-3 right-4">
                    <Badge variant="outline" className={`${tierConfig.accentBorder} ${tierConfig.accentText} bg-[#0D0D0D]`}>
                      Current
                    </Badge>
                  </div>
                )}
                ...
                <CardHeader className="text-center pt-8">
                  <div className="flex justify-center mb-3">
                    <div className={`p-3 rounded-full bg-gradient-to-b ${tierConfig.accentBg}`}>
                      <Icon className={`w-6 h-6 ${tierConfig.accentText}`} />
                    </div>
                  </div>
                  <CardTitle className={`text-xl font-bold ${tierConfig.accentText}`}>
                    {tierConfig.name}
                  </CardTitle>
                </CardHeader>
                  ...
                  <div className="mb-6">
                    <div className="flex items-baseline justify-center gap-1">
                      <span className="text-4xl font-bold text-white">
                        {isAnnual ? tierConfig.annualMonthly : tierConfig.monthlyPrice}
                      </span>
                  ...
                  <ul className="space-y-3 text-left">
                    {tierConfig.features.map((feature) => (
                      <li key={feature.label} className="flex items-start gap-2">
                        <Check className={`w-4 h-4 mt-0.5 shrink-0 ${tierConfig.accentText}`} />
                        <span className="text-sm text-[#E5E7EB]">{feature.label}</span>
                      </li>
                    ))}
                  </ul>
                </CardContent>
