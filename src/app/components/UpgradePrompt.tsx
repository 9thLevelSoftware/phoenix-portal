...
import { Lock, Crown, Flame, Zap } from 'lucide-react';
...
const TIER_COLORS: Record<PaidTier, { border: string; glow: string; accent: string }> = {
  PHOENIX: {
...
export function UpgradePrompt({ requiredTier, currentTier: _currentTier, featureName }: UpgradePromptProps) {
  ...
  return (
    ...
    >
      ...
      <div className="relative z-10 flex flex-col items-center text-center gap-5">
        {/* Lock icon */}
        <div className={`rounded-full bg-[#0D0D0D] border ${colors.border} p-4`}>
        ...
        <div>
          <h3 className="text-lg font-semibold text-zinc-100 mb-1">
            Upgrade to{' '}
            <span className={colors.accent}>{requiredTier}</span>
          </h3>
          <p className="text-sm text-zinc-400">
            Unlock {featureName ?? 'this feature'} and more with a {requiredTier} subscription.
          </p>
        </div>
        ...
        <ul className="space-y-3 w-full max-w-xs text-left">
          {benefits.map((benefit) => (
            <li key={benefit.label} className="flex items-center gap-3 text-sm text-zinc-300">
