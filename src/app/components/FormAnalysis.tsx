...
function getSubMetricDescription(name: string, score: number): string {
	...
	if (name === "Tempo Control") {
		if (score >= 80)
			return "Time under tension is consistent throughout the set.";
	...
	if (name === "Fatigue Resistance") {
		...
		if (score >= 60)
			return "Slight performance drop toward the end of the set.";
...
export function FormAnalysis({ reps }: FormAnalysisProps) {
	const { overallScore, grade, subMetrics, recommendations } = useMemo(() => {
		...
		const metrics: SubMetric[] = [
			{
				name: "Curve Consistency",
				score: cc,
				description: getSubMetricDescription("Curve Consistency", cc),
			},
			{
				name: "Tempo Control",
				score: tc,
				description: getSubMetricDescription("Tempo Control", tc),
			},
			{
				name: "Fatigue Resistance",
				score: fr,
				description: getSubMetricDescription("Fatigue Resistance", fr),
			},
			{
				name: "Bilateral Balance",
				score: bb,
				description: getSubMetricDescription("Bilateral Balance", bb),
			},
		];
