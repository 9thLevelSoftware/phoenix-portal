...
export function CycleBuilderMain({
	cycleId,
	onSave,
	onCancel,
}: CycleBuilderProps) {
	...
	useEffect(() => {
		if (duration !== days.length) {
			if (duration > days.length) {
				...
				setDays(
					days.slice(0, duration).map((d, i) => ({ ...d, dayNumber: i + 1 })),
				);
			}
			...
			{selectedDay && selectedDayData && (
				<DayEditor
					...
					onUpdateOverrides={(overrides: DayOverrides) => {
						setDays(
							days.map((d) =>
								d.dayNumber === selectedDay ? { ...d, overrides } : d,
							),
						);
						setHasUnsavedChanges(true);
					}}
					onUpdateNotes={(notes: string) => {
						setDays(
							days.map((d) =>
								d.dayNumber === selectedDay ? { ...d, notes } : d,
							),
						);
						setHasUnsavedChanges(true);
					}}
					onUpdateRestType={(type: "complete" | "active" | "mobility") => {
						setDays(
							days.map((d) =>
								d.dayNumber === selectedDay ? { ...d, restType: type } : d,
							),
						);
						setHasUnsavedChanges(true);
					}}
