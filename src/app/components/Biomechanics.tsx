...
import { workoutListOptions, sessionDetailOptions } from '@/queries/workouts';
import { repTelemetryOptions, repSummariesOptions } from '@/queries/telemetry';
...
function BiomechanicsContent() {
  ...
  const effectiveSessionId =
    selectedSessionId || (workouts?.[0]?.id ?? '');
  if (effectiveSessionId && !selectedSessionId && workouts && workouts.length > 0) {
    ...
    <div className="space-y-6">
      ...
      {sessionLoading ? (
        ...
        <>
          ...
          <Section title="Force Curves" icon={Activity} className="col-span-full">
          ...
          <Section title="Exercise Progress" icon={Activity} className="col-span-full">
            <ExerciseProgress
              userId={userId}
          ...
          <Section title="Summary Report" icon={Activity} className="col-span-full">
            <SummaryReport userId={userId} />
          </Section>

          {/* Section 8: Consistency (full width) */}
          <Section title="Workout Consistency" icon={Activity} className="col-span-full">
