...
export function weeklySummaryOptions(
	...
	return queryOptions({
		...
		queryFn: async () => {
			...
			// client-readable (20260923100000), so `*` would be refused.
			let query = supabase
				.from("exercise_progress")
				.select(
					"id, user_id, exercise_name, session_id, recorded_at, max_weight_kg, total_volume_kg, estimated_1rm_kg, max_reps, set_count",
				)
				.eq("user_id", userId);
