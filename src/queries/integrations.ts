  ...
  return queryOptions({
    ...
    queryFn: async () => {
      const { data, error } = await supabase
        ...
        .select('id, user_id, provider, provider_user_id, connected_at, last_sync_at, status, error_message')
        .eq('user_id', userId)
 ...
 */
export function externalActivitiesOptions(userId: string, provider?: IntegrationProvider) {
  return queryOptions({
    queryKey: provider
