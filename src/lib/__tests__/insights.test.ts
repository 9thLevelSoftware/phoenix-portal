...
describe("generateInsights", () => {
  ...
  it("flags volume decrease as warning", () => {
    ...
    const volumeDown = insights.find((i) => i.title.includes("Volume") && i.type === "warning");
  ...
  it("flags low consistency", () => {
    ...
    const insights = generateInsights(input);
    const consistency = insights.find((i) => i.title.includes("Consistency") || i.title.includes("consistency"));
  ...
  it("flags streak milestones", () => {
    ...
    const insights = generateInsights(input);
    const streak = insights.find((i) => i.type === "achievement" && (i.title.includes("Streak") || i.title.includes("streak")));
  ...
  it("flags high training load", () => {
    ...
    const insights = generateInsights(input);
    const load = insights.find((i) => i.title.includes("Load") || i.title.includes("load"));
