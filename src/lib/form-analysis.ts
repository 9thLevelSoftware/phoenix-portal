...
export function calculateCurveConsistency(reps: RepMetrics[]): number {
	...
	if (mean === 0) return 0;
	const variance =
		forces.reduce((sum, f) => sum + (f - mean) ** 2, 0) / forces.length;
...
export function calculateTempoControl(reps: RepMetrics[]): number {
	...
	if (mean === 0) return 0;
	const variance =
		tuts.reduce((sum, t) => sum + (t - mean) ** 2, 0) / tuts.length;
	const cv = Math.sqrt(variance) / mean;
...
export function calculateBilateralBalance(reps: RepMetrics[]): number {
	...
	if (asymmetries.length >= 3) {
		...
		const firstThird = asymmetries.slice(
			0,
			Math.ceil(asymmetries.length * 0.33),
		);
	...
	return Math.max(
		0,
		Math.min(100, Math.round(100 - avgAsymmetry * 5 - trendPenalty * 3)),
	);
}
